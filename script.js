...
document.getElementById("fondo-img").addEventListener("change", function () {
  const file = this.files[0];
  const reader = new FileReader();
  reader.onload = function (e) {
    let config = JSON.parse(localStorage.getItem("personalizacion")) || {};
    config.fondoImg = e.target.result;
...
function aplicarPersonalizacion() {
  ...
  if (config) {
    ...
    document.body.style.fontFamily = config.fuente;
    if (config.fondoImg) {
      document.body.style.backgroundImage = `url(${config.fondoImg})`;
      document.body.style.backgroundSize = "cover";
    }

    // Solo rellenar si el admin está logueado
    if (document.getElementById("color-fondo")) {
      document.getElementById("color-fondo").value = config.fondo;
      document.getElementById("color-texto").value = config.texto;
      document.getElementById("fuente").value = config.fuente;
    }
...
function enviarWhatsApp() {
  ...
  const numero = prompt("Ingresa tu número de WhatsApp:");
  if (!numero) return;

  navigator.geolocation.getCurrentPosition(pos => {
    const linkUbicacion = `https://maps.google.com/?q=${pos.coords.latitude},${pos.coords.longitude}`;
    const mensaje = encodeURIComponent(
      `Pedido:
` + carrito.map(p => `${p.nombre} x${p.cantidad} - $${p.precio * p.cantidad}`).join("
") +
      `
Total: $${carrito.reduce((a, p) => a + p.precio * p.cantidad, 0)}
