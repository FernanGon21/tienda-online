...
document.getElementById("enviar-whatsapp").onclick = () => {
  ...
  const url = `https://wa.me/595985718172?text=${encodeURIComponent(mensaje)}`;
  window.open(url, '_blank');
  alert("¡Tu pedido fue enviado por WhatsApp!");
  carrito = [];
  document.getElementById("carrito-contador").textContent = "0";
  document.getElementById("carrito-lista").innerHTML = "";
  let totalHTML = carrito.reduce((sum, p) => sum + p.precio, 0);
  const delivery = parseInt(document.getElementById("delivery").value) || 0;
  const totalConDelivery = totalHTML + delivery;
  document.getElementById("carrito-total").textContent = "Total + Delivery: Gs. " + totalConDelivery;
